...
func configRAMPolicy(region, akid, aksecret string) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to create RAM client")
	}
	...
	// Create AliyunECSImageImportDefaultRole if not exist
	{
		...
		if response == nil {
			...
			if err != nil {
				return errors.Wrapf(err, "Failed to create RAM role")
			}
		}
	}
	...
	// Attach AliyunECSImageImportRolePolicy if not exist
	{
		...
		if err != nil {
			return errors.Wrapf(err, "Failed to list policies for role")
		}
		...
		if err != nil {
			return errors.Wrapf(err, "Failed to attach policy to role")
		}
	}
	return nil
}
...
func createECSImage(region, akid, aksecret, name, bucketname string, imagesize int, uefi, nvme bool) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to initilize ECS client")
	}
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to create ECS image")
	}
...
func pushAlibabaCloudCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if akidFlag == "" {
				return errors.Errorf("Missing required flag --access-key-id")
			}

			aksecretFlag = getStringValue(accessKeySecretVar, aksecretFlag, "")
			if aksecretFlag == "" {
				return errors.Errorf("Missing required flag --access-key-secret")
			}

			regionFlag = getStringValue(regionIDVar, regionFlag, "")
			if regionFlag == "" {
				return errors.Errorf("Missing required flag --region")
			}

			if bucketFlag == "" {
				return errors.Errorf("Missing required flag --bucket")
			}
